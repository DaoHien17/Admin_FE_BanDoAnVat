...
export class NhacungcapComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  public openUpdateModal(maSanPham: any) {
    ...
    setTimeout(() => {
      $('#createSanPhamModal').modal('toggle');
      this._api.get('/api/nhacungcap/get-by-id/' + maSanPham).subscribe(res => {
        this.sanpham = res[0];
        this.frmLoaiSanPham = new FormGroup({
          'txt_tenncc': new FormControl(this.sanpham.TenNhaCungCap, [Validators.required, Validators.minLength(3), Validators.maxLength(250)]),
          'txt_diachi': new FormControl(this.sanpham.DiaChi, [Validators.required]),
          'txt_sodienthoai': new FormControl(this.sanpham.SoDienThoai, [Validators.required]),
          'txt_email': new FormControl(this.sanpham.Email, [Validators.required]),
        });
        this.doneSetupForm = true;
      });
    });
